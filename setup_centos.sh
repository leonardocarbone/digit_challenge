...
sudo docker run -v /tmp/jenkins_backup:/jenkins_backup -v /var/run/docker.sock:/var/run/docker.sock -v $(which docker):/usr/bin/docker -p 8080:8080 --rm --name jenkins-test-digit jenkins-test-digit 'invoke restore_run jenkins.bkp.tar.gz'